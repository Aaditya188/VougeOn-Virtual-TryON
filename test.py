...
def get_face_boundbox(points, face_part):
    if face_part == 1:
        ...
        (x,y,w,h) = calculate_boundbox(points[5:17])
    return (x,y,w,h)
    ...
    while run_event.is_set(): 
        ...
        for face in faces: 
            ...
            if SPRITES[2]:
                ...
                (x3,y3,w3,h3) = get_face_boundbox(shape, 7) #nose
                apply_sprite(image, IMAGES[2][ACTIVE_IMAGES[2]],w3,x3+120,y3+75, incl) 
...
#                apply_sprite(image, IMAGES[5][ACTIVE_IMAGES[5]],w,x,y, incl, ontop = True)
            ...
            if SPRITES[5]:
                ...
                apply_sprite(image, IMAGES[5][ACTIVE_IMAGES[5]],w1+200,x1-90,y1+50, incl,  ontop = False)
            ...
            if SPRITES[4]:
                ...
                # apply_sprite2feature(image, IMAGES[7][ACTIVE_IMAGES[7]], fullbody, w//4, 2*h//3, h//2, True, w//2, x, y, w, h)
                (x1,y1,w1,h1) = get_face_boundbox(shape, 6)
                apply_sprite(image, IMAGES[4][ACTIVE_IMAGES[4]],w1+380,x1-180,y1+80, incl,  ontop = False)
...
top_frame = Frame(root, bg='#15939e', width=50, height=50, pady=3)
center = Frame(root, bg='#15939e', width=50, height=40, padx=3, pady=3)
btm_frame = Frame(root, bg='#15939e', width=50, height=50, pady=3)
...
ctr_left = Label(center,bg='#15939e', width=50, height=190)
ctr_mid = Label(center,bg='#15939e',width=100, height=160, padx=0, pady=0)
...
for index in range(9):
    if len(PHOTOS[index]) > 0:
        for k,photo in enumerate(PHOTOS[index]):
            btn= Button(scrollable_body, highlightbackground='#15939e', text=IMAGES[index][k].rsplit('/',1)[1].replace('.png','')[:-1],bg='#15939e', image=photo, command = lambda index=index, k=k: put_sprite(index,k), compound=LEFT, width='170', height='220')
